...
Background compaction is **off by default**. To opt in, stop the running server with its matching client, then launch with `MORONS_BACKGROUND_COMPACTION=1 morons`. This permits additional inference using the successful run's exact service/model and billing identity; unused or discarded summaries can still consume quota or money. Other values leave it off. To disable it, stop through the authenticated client and restart without the flag. No other application's login or credentials are imported.
