...
Release archives include a checksummed `morons-uv` helper. On the first `ipython` call, the companion uses it to prepare Morons-owned Python 3.11.15 with hash-locked `jupyter_client` 8.6.3 and `ipykernel` 6.30.1. Windows ARM64 uses x86_64 CPython through Windows' built-in emulation because native PyZMQ cannot currently complete the Jupyter handshake; Morons and its packaged companions remain native ARM64. Initial setup requires internet access to the reviewed Python and PyPI sources. The versioned runtime and download cache live under `~/.morons/python` on macOS/Linux or `%LOCALAPPDATA%\\morons.dev\\python` on Windows; later use works without network access. Interrupted, stale, or invalid staging state is rebuilt under a process lock and never becomes the active runtime.
