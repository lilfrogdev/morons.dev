...
Packaged use does not require Rust or a preinstalled Python. Bash is required for the `bash` tool and `!`/`!!` command modes; on Windows, Morons discovers a normal Git for Windows installation or uses the expert `MORONS_BASH` override set before the companion starts. Model inference requires the selected provider's credential: an OpenCode Zen/Go API key or native ChatGPT login. Neither is required for launch or local session management. First managed-IPython setup and provider operations require network access. `web_search` prefers OpenAI-hosted search through ChatGPT, including when the coding model uses OpenCode. A binding created with definitely absent ChatGPT credentials may use keyless Exa only with both privacy restrictions off; policy denial and dispatched failures never authorize fallback. There is no Brave executor. See the separate search identity and usage below.
...
The Exa route sends only the query and fixed search options to the reviewed keyless endpoint and returns attributed source excerpts without invented token usage. Both routes record admission before HTTP and success before returning results; uncertain attempts are consumed, not replayed. Search integration remains under qualification; offline checks do not establish live compatibility. See [integration status](docs/architecture/pi-web-access-integration.md). Historical Brave results are read-only compatibility, not an available backend.
...
Canonical transcript history remains durable. The terminal pages through complete delivered transcript text in bounded parts, rather than discarding a long entry's tail. Windows have independent entry-count and 1 MiB assembled-source bounds; byte pressure may produce a smaller window. Home/End, paging and wheel navigation retain adjacent history cursors. New output does not steal a historical reader's position. A preview that overflows or loses delta continuity pauses until the complete committed message arrives. This exposes delivered DTO text, not previously uncaptured or unreturned tool output; see [ADR 0051](docs/adr/0051-navigable-transcript-text.md).

Automatic and manual compaction create source-bound lossy summaries without deleting canonical messages or attachments. Old accepted runs retain legacy whole-current-run protection. New eligible native text roots use [execution policy1](docs/adr/0052-context-accounting-policy.md): compatible immediately preceding committed same-run usage may inform admission, and completed tool batches may be compacted while retaining the exact original user intent and latest batch. At most **four foreground compaction attempts per run**, including initial/manual compaction, are allowed with advancing source and intervening completed coding progress. Summaries are extra provider requests outside the completed coding-turn counter; neither counter is a spending cap. After checkpoint commit, ephemeral continuation is reset.
...
For opt-in diagnostics, stop the existing server with Ctrl+S when work is idle, then run `morons --debug`. Sanitized diagnostics go to UTC daily files in `~/.morons/logs` on Unix (LocalAppData/morons.dev/logs on Windows); reconnect with plain `morons`. An existing server is never restarted or switched into debug mode automatically. `morons-server --debug` uses the same files. `morons --debug-status` reports logging health and the log directory from the existing server without starting one. Logs are bounded and lossy; missing receipts mean unknown usage, not zero. See [debug logging](docs/debug-logging.md) and [testing](docs/testing.md#opt-in-provider-debugging).
...
Drag with the left mouse button to select visible text anywhere in the terminal UI;
