...
The most recently selected or used reviewed model is the global default across sessions and client restarts. Opening an older session does not restore that session's historical model. If the saved default is unavailable, Morons uses another currently available reviewed model and reports the fallback. Each service/model pair pins its reviewed wire protocol. The built-in Go snapshot covers all 35 identifiers exposed by the public Go catalog at review time across bounded Responses, Chat Completions, and Anthropic Messages adapters; the live catalog may only mark those entries available or unavailable. Training-eligible, non-ZDR, and privacy-undocumented models remain visible with explicit policy disclosures rather than being silently omitted or assigned a favorable classification.
