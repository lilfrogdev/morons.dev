...
fn build(child: bool) -> String {
    ...
    if child {
        text.push_str("\nChild execution: there is no cumulative turn, tool-call or mutation quota or whole-assignment deadline. Per-operation and context limits still apply. Batch independent calls when appropriate and finish with a concise report; report partial progress instead of repeating work. Check the latest tool result before deciding whether another action is needed.");
    }
    text.push_str("\n\nTool guidance:");
    for kind in [
        ToolKind::Read,
...
mod tests {
    ...
    fn shared_core_and_role_specific_tools_do_not_leak_parent_workflow_to_children() {
        for child in [false, true] {
            ...
            assert!(prompt.contains("Child execution:") == child);
