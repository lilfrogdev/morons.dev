    ...
    let invalid: bool = connection.query_row(
        "SELECT EXISTS (
            ...
            WHERE NOT (
                ...
                OR
                (accepted.tool_catalog_version IN (3, 4, 5, 6, 7, 8, 9, 10, 11)
            ...
            WHERE call.session_id IS NOT run.session_id
               ...
               OR (call.tool_kind BETWEEN 8 AND 10 AND run.tool_catalog_version NOT IN (3, 4, 5, 6, 7, 8, 9, 10, 11))
               OR (call.tool_kind = 11 AND run.tool_catalog_version NOT IN (4, 5, 6, 7, 8, 9, 10, 11))
               OR (call.tool_kind = 12 AND run.tool_catalog_version NOT IN (5, 6, 7, 8, 9, 10, 11))
               OR (call.tool_kind = 13 AND run.tool_catalog_version NOT IN (6, 7, 8, 9, 10, 11))
               OR (call.tool_kind = 14 AND run.tool_catalog_version NOT IN (8, 9, 10, 11))
               OR (call.tool_kind BETWEEN 1 AND 7 AND run.tool_catalog_version IN (3, 4, 5, 6, 7, 8, 9, 10, 11))
