...
const SCHEMA_V43: &str = include_str!("../schema_v43.sql");
const SCHEMA_V42: &str = include_str!("../schema_v42.sql");
const SCHEMA_V41: &str = include_str!("../schema_v41.sql");
const SCHEMA_V35: &str = include_str!("../schema_v35.sql");
const SCHEMA_V36: &str = include_str!("../schema_v36.sql");
const SCHEMA_V37: &str = include_str!("../schema_v37.sql");
const SCHEMA_V38: &str = include_str!("../schema_v38.sql");
const SCHEMA_V39: &str = include_str!("../schema_v39.sql");
const SCHEMA_V40: &str = include_str!("../schema_v40.sql");

const EXPECTED_SCHEMA_OBJECTS: &[(&str, &str)] = &[
    ("steering_delivery_facts", "table"),
    ("steering_mutation_requests", "table"),
    ("steering_lifecycle_facts", "table"),
    ("steering_queues", "table"),
    ("steering_pending_messages", "table"),
    ("run_accepted_facts_by_session_run", "index"),
    ...
    ("context_accounting_epoch", "table"),
    ...
    ("subagent_model_selections_by_sequence", "index"),
    ...
    ("web_search_attempts", "table"),
    ("web_provenance_epoch", "table"),
    ("web_binding_provenance", "table"),
    ("web_search_successes", "table"),
    ("web_attempt_epoch", "table"),
...
fn initialize_at_path(
    ...
    connection.execute_batch(SCHEMA_V34_SEARCH)?;
    connection.execute_batch(SCHEMA_V35)?;
    connection.execute_batch(SCHEMA_V36)?;
    connection.execute_batch(SCHEMA_V37)?;
    connection.execute_batch(SCHEMA_V38)?;
    connection.execute_batch(SCHEMA_V39)?;
    connection.execute_batch(SCHEMA_V40)?;
    connection.execute_batch(SCHEMA_V41)?;
    connection.execute_batch(SCHEMA_V42)?;
    connection.execute_batch(SCHEMA_V43)?;
...
fn migrate(connection: &Connection, paths: &StoragePaths) -> Result<(), PersistenceError> {
    ...
    for (version, schema) in [
        ...
        if version > schema_version {
            if version == 35 {
                let has_search: bool = connection.query_row(
                    "SELECT EXISTS(SELECT 1 FROM pragma_table_info('web_model_bindings') WHERE name = 'exa_contract_revision')",
                    [], |row| row.get(0),
                )?;
                if !has_search {
                    migrate_schema(connection, SCHEMA_V34_SEARCH)?;
                }
            }
