...
impl Backend {
    ...
    pub(crate) fn clean_session_database(
        ...
        for statement in [
            ...
            "DELETE FROM web_search_successes WHERE call_id IN (SELECT call_id FROM web_model_bindings WHERE run_id IN (SELECT run_id FROM run_accepted_facts WHERE session_id = ?1))",
