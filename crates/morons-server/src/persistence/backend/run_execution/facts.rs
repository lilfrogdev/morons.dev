...
pub(crate) fn append_run_transition(
    ...
    if state.is_terminal() {
        super::super::steering_lifecycle::pause(
            transaction,
            Some(run.session_id.as_bytes()),
            Some(run.id.as_bytes()),
            super::super::steering_lifecycle::PauseReason::Terminal,
            Some(fact_sequence),
            now,
        )?;
    }
...
pub(super) fn insert_provider_prepared(
    ...
    transaction.execute(
        ...
        params![
            ...
            sequence_to_sql(fact.turn_index)?,
