...
CREATE TABLE run_accepted_facts_v47 (
    ...
    estimated_input_tokens INTEGER NOT NULL CHECK (estimated_input_tokens BETWEEN 1 AND (CASE WHEN open_code_service = 3 AND protocol_revision = 5 AND model_id IN ('gpt-5.5','gpt-6-astra','gpt-5.6-sol','gpt-5.6-luna','gpt-5.6-terra','gpt-daybreak-blue-latest') THEN 258400 ELSE 96000 END)),
    maximum_input_tokens INTEGER NOT NULL CHECK (maximum_input_tokens BETWEEN 1 AND (CASE WHEN open_code_service = 3 AND protocol_revision = 5 AND model_id IN ('gpt-5.5','gpt-6-astra','gpt-5.6-sol','gpt-5.6-luna','gpt-5.6-terra','gpt-daybreak-blue-latest') THEN 258400 ELSE 96000 END)),
...
INSERT INTO run_accepted_facts_v47 SELECT * FROM run_accepted_facts;

DROP TABLE run_accepted_facts;

ALTER TABLE run_accepted_facts_v47 RENAME TO run_accepted_facts;
...
INSERT INTO runs_v47 SELECT * FROM runs;

DROP TABLE runs;

ALTER TABLE runs_v47 RENAME TO runs;

CREATE INDEX runs_by_session ON runs (session_id, accepted_sequence);

CREATE TABLE task_model_bindings_v47 (
    ...
    credential_generation INTEGER NOT NULL CHECK (credential_generation > 0),
    protocol_revision INTEGER NOT NULL CHECK (protocol_revision BETWEEN 1 AND 65535),
    maximum_input_tokens INTEGER NOT NULL CHECK (maximum_input_tokens BETWEEN 1 AND (CASE WHEN service = 3 AND protocol_revision = 5 AND model_id IN ('gpt-5.5','gpt-6-astra','gpt-5.6-sol','gpt-5.6-luna','gpt-5.6-terra','gpt-daybreak-blue-latest') THEN 258400 ELSE 96000 END)),
    maximum_output_tokens INTEGER NOT NULL CHECK (maximum_output_tokens BETWEEN 1 AND 32000),
...
INSERT INTO task_model_bindings_v47 SELECT * FROM task_model_bindings;

DROP TABLE task_model_bindings;

ALTER TABLE task_model_bindings_v47 RENAME TO task_model_bindings;

CREATE INDEX task_model_bindings_by_run ON task_model_bindings(run_id, dispatch_sequence);

CREATE TABLE provider_operation_facts_v47 (
    fact_id BLOB PRIMARY KEY NOT NULL CHECK (length(fact_id) = 16),
    fact_sequence INTEGER NOT NULL UNIQUE CHECK (fact_sequence > 0),
    operation_id BLOB NOT NULL CHECK (length(operation_id) = 16),
    run_id BLOB NOT NULL REFERENCES run_accepted_facts(run_id),
...
CHECK (estimated_input_tokens IS NULL OR estimated_input_tokens BETWEEN 1 AND (CASE WHEN open_code_service = 3 AND protocol_revision = 5 AND model_id IN ('gpt-5.5','gpt-6-astra','gpt-5.6-sol','gpt-5.6-luna','gpt-5.6-terra','gpt-daybreak-blue-latest') THEN 258400 ELSE 96000 END)),
...
INSERT INTO provider_operation_facts_v47 SELECT * FROM provider_operation_facts;

DROP TABLE provider_operation_facts;

ALTER TABLE provider_operation_facts_v47 RENAME TO provider_operation_facts;
