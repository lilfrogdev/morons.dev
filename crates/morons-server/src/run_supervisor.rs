...
fn build_provider_request(
    ...
    let tools_enabled = context.run.tool_catalog_version == TOOL_CATALOG_VERSION
        && context.run.tool_limits_version == crate::tools::TOOL_LIMITS_VERSION;
    if (
...
fn normalize_provider_turn(
    ...
    let calls = parse_provider_calls(calls).map_err(|error| match error {
