...
async fn proactive_compaction_commits_a_source_bound_summary_and_uses_recent_tail() {
    ...
    let session = store
        ...
        .await
        .expect("session should be created");
    append_completed_context_run(&store, session.id, 0xe3, "OLD_RUN_ONE").await;
    let hidden = store
    ...
    append_completed_context_run(&store, session.id, 0xe4, "RECENT_RUN_TWO").await;
    append_completed_context_run(&store, session.id, 0xe5, "RECENT_RUN_THREE").await;
    ...
    store
        .complete_run_success(
            ...
            crate::persistence::CompletedAssistant {
                text: format!("{marker}_ASSISTANT {}", "x".repeat(24_000)),
                refusal: false,
