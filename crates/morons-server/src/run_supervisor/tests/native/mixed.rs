...
async fn mixed_provider_children_pin_their_own_generations_and_ignore_later_model_changes() {
    for native_parent in [false, true] {
        ...
        let server = tokio::spawn(async move {
            ...
            for step in 0..4 {
                ...
                let native = if child { !native_parent } else { native_parent };
                assert!(request.starts_with(if native {
                ...
                assert_eq!(
                    body["model"],
                    if native { "gpt-5.5" } else { "muse-spark-1.2" }
                );
                ...
                if native {
                    write_native(&mut stream, &format!("resp_{step}"), &output).await;
                } else {
        ...
        assert_eq!(
            binding,
            (
                if native_parent { 1 } else { 2 },
                if native_parent { 1 } else { 3 },
                child_model.into()
            )
        );
        let call: [u8; 16] = db
