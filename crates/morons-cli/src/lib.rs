...
impl Error for HandshakeError {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match self {
            ...
            Self::ServerDisconnected | Self::ProtocolVersionMismatch { .. } => None,
        }
    }
}
...
/// Performs one client-side handshake and returns the accepted server version.
...
mod tests {
    use morons_protocol::{
        ClientMessage, PROTOCOL_VERSION, ServerMessage, read_client_message, write_server_message,
    };
