...
For an explicitly authorized foreground measurement, run `morons-server --debug 2>/absolute/owner-controlled/path/startup.stderr` with a stopped server and the intended profile; keep the capture outside the repository. This does not activate diagnostics in an already running server, change daemon logging, or authorize stopping one. Normal startup remains silent on this sink.
