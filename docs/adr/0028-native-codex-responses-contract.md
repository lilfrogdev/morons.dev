...
The existing strict Responses SSE state machine and input/tool/image validators are reused without relaxing OpenCode behavior. [ADR0037](0037-native-stream-media-type-compatibility.md) subsequently permits an omitted Content-Type on native HTTP200 streaming responses; a present conflicting type still rejects and the shared HTTP helper remains strict. Exact model/lifecycle/sequence, complete attributed output, usage arithmetic, terminal state, bounded opaque reasoning and UTF-8 remain required. [ADR0038](0038-native-completed-output-items.md) subsequently adds native-only bounded complete-item collection for empty terminal output, retaining these checks and the old OpenCode constructor. Native bodies and provider headers do not select capabilities or destinations.
