...
Morons captures attachment bytes immediately so temporary pasteboard and drag paths are not required after ingestion. The client uses the platform clipboard directly for `Ctrl+V` (`Alt+V` on Windows), conservatively recognizes pasted, quoted, escaped, and `file://` image paths, and otherwise preserves paste as text. It detects type from content, applies EXIF orientation, validates dimensions, bounds animated GIF frames, and normalizes supported PNG, JPEG, WebP, and GIF inputs to bounded PNG, JPEG, or GIF payloads. Each attachment records its sanitized filename, media type, normalized dimensions, size, digest, prompt marker position, and session-owned storage reference.

Normalized attachment bytes are stored once in owner-only Morons attachment files named only by random identifiers; SQLite stores bounded metadata and canonical user-message or `read`-tool references. Run acceptance binds attachment order and digests into idempotency state, and startup validates referenced files while removing only unreferenced files from the Morons-owned attachment directory. Provider adapters map provider-neutral text and image parts to bounded data URLs only for models with a reviewed image-input capability. If the selected model lacks image input, submission fails clearly while retaining the client draft and attachments; images are never silently discarded. The `read` tool similarly returns a durable file-backed normalized image for vision models and a clear unsupported result otherwise.
...
The policy reserves model-specific space for output, tool schemas, and bounded tool-loop growth. Under context policy v4, Morons asks the selected model to summarize an old complete prefix while preserving the current run. [ADR 0022](0022-release-context-and-provider-hardening.md) adds independent entry/image pressure and retains two recent complete prior turns when they fit, shortening that tail before a hard limit. [ADR 0023](0023-context-observation-and-request-preparation.md) uses compatible provider usage plus a bounded tail for the seventy-percent proactive threshold, without relaxing conservative dispatch guards. The checkpoint records the exact source-entry high water and digest, policy version, bounded summary, token estimate, selected service and model, and parent lineage. The summary request has no tools, is never retried after dispatch, excludes `!!` content, and has a bounded output allowance. A failed or invalid compaction leaves canonical history unchanged and fails or interrupts the owning run without replaying the request.
...
Direct local effects do not create the old workspace-generation uncertainty blocker because there is no authoritative private generation to reconcile. The user and a later model inspect the actual selected directory with ordinary tools. External side effects are never retried automatically after an unknown outcome. The application protocol and terminal client therefore expose no workspace-state summary, repository-import progress, workspace-change event, or uncertainty-acknowledgement mutation. Historical workspace and acknowledgement facts remain strictly decoded for migration, integrity validation, projection repair, and deletion; historical workspace delivery events advance resumable internal cursors without being published to current clients.
