...
The policy reserves model-specific space for output, tool schemas, and bounded tool-loop growth. Under context policy v4, when projected input reaches seventy percent of the selected model's input limit, Morons asks that model to summarize the oldest complete prefix while retaining the current run and at least the two newest prior user turns verbatim. The checkpoint records the exact source-entry high water and digest, policy version, bounded summary, token estimate, selected service and model, and parent lineage. The summary request has no tools, is never retried after dispatch, excludes `!!` content, and has a bounded output allowance. A failed or invalid compaction leaves canonical history unchanged and fails or interrupts the owning run without replaying the request.
