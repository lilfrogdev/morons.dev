...
- Status: Accepted — opt-in supervised runtime and first-request installation
- Date: 2026-09-05
...
- Begin with an explicitly disclosed, owner-controlled opt-in, default off. Preparing or discarding a summary can still consume provider quota or incur cost. Turning it off prevents new jobs and cancels/discards current work safely.
...
The first opt-in is the server-startup environment value `MORONS_BACKGROUND_COMPACTION=1`; other values leave it off. This is not a model/tool argument or an imported configuration. The server and storage worker capture the owner setting once. `/context` discloses enabled state, the last job's state, exact service/model and bounded maintenance observations separately from foreground usage. Enabling permits additional billable inference, including unused/discarded results. To disable, stop through authenticated IPC and restart without the flag; shutdown drains the controlled request and disabled startup discards retained readiness. No live settings toggle is introduced in this increment.
