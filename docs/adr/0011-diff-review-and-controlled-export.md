...
The destination must not exist and must not overlap Morons application, control, runtime, data, backup, credential, image, operation, or workspace roots. Its existing parent chain must be ordinary and free of links or reparse points. Morons never accepts overwrite, merge, conflict resolution, source write-back, Git metadata, ownership, ACL, extended-attribute, alternate-stream, sparse-file, or special-permission options.

Under the session workspace lease, trusted server code revalidates the reviewed generation, copies it into an operation-named sibling staging directory with the repository import path and resource policy, strips untrusted auxiliary metadata, synchronizes the complete tree, and verifies a canonical manifest. It then commits dispatch and atomically renames the sibling staging directory to the previously absent destination. The parent directory is synchronized where the platform supports it. Export never executes repository code, Git, hooks, or a provider request.
...
Export records prepared, dispatched, completed, not-applied, or uncertain facts plus idempotency and structured audit facts. Prepared state binds the session, active generation, generation manifest, destination-path digest, operation identity, format, and limits without storing the path.

Startup performs no external destination lookup because the destination path is not durable. Prepared but undispatched export state becomes not applied and server-private staging is removed by exact identity. Dispatched state without a committed outcome becomes uncertain and is never retried, completed, removed, or described as absent automatically. It does not block workspace mutation because export cannot change authoritative session state, but its audit history remains uncertain.

An exact live retry may use the same transient destination path and request identity to inspect only its exact operation-named sibling staging and final destination. It completes only when the reviewed generation manifest and destination-path digest prove the exact full export; otherwise it retains the uncertain result. A new export requires a new absent destination and mutation identity.
