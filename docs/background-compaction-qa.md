...
Regressions cover low advisory usage near the independent conservative guard; startup defaults/opt-out and stale preparation; duplicate/uncertain prefix attempts; exact binding and recovery; overlapping or abandoned drain callers; nonqueueing lifecycle admission; late/manual/incompatible readiness; hard receiving-tail rejection; immutable prepared bytes; actual next-request installation; blocked-request deadline/stop/archive and archived-session deletion. The archive test also verifies that deletion of an unarchived session is rejected rather than bypassing lifecycle rules.

## Credentialed development procedure

Obtain explicit approval for billable requests and a bounded request budget first. Prefer fresh, owner-controlled Morons state and a separate disposable selected directory. Reusing a retained Morons QA home requires explicit approval, a planned schema upgrade with a verified pre-upgrade database backup, and preservation of its credentials, old sessions and diagnostic fixtures. Do not import credentials from another application, copy credential files, migrate retained diagnostic state casually, or delete/log out retained QA state. Enter any new key only in the non-echoing authenticated `/login` dialog; do not capture that dialog. No prompts, tool arguments, provider bodies, hidden reasoning, opaque identifiers or credentials belong in diagnostic logs.

1. Record the exact source/build and matching client/server protocol. Launch the QA home with `MORONS_BACKGROUND_COMPACTION=0`; `/context` must show disabled and normal short input must not create maintenance work.
2. Stop that QA server through its authenticated client. Restart the same QA home with `MORONS_BACKGROUND_COMPACTION` unset to test the enabled default, deliberately authorizing possible charges for unused summaries. Verify `/context` shows the disclosure.
...
4. Observe the latest maintenance state, source cutoff and completed usage through `/context` (reopen to refresh). A ready result must not yet change the active checkpoint. Once Ready, submit a small new input; Installed and a matching checkpoint cutoff should be visible afterward. Check the final response for retained requirements, not merely a model claim that compaction worked. Verify the receiving run does not immediately add a foreground compaction and replace that checkpoint; Installed alone is insufficient evidence of useful reuse. A wrong recalled requirement is a failed semantic check even if the run succeeded.
...
6. Exercise new input during dispatch and authenticated stop while busy. Input acceptance must not queue behind inference, but existing credential coordination may delay foreground dispatch while response headers are pending. `Esc` cancels an exact foreground run, not independent maintenance; stop the isolated server to drain background work. Cancellation cannot retract remote processing or charges.
7. Restart with `MORONS_BACKGROUND_COMPACTION=0` and verify disabled state and no readiness installation or automatic replay. Archive before deleting a disposable session; verify selected files and guidance remain unchanged. Preserve failed fixtures and captures for diagnosis.
...
Live qualification requires an authorized Morons login in the approved QA state. If that is unavailable, report the gate as blocked; do not substitute an existing application's credentials or claim loopback evidence is live QA. Follow the separate [release qualification procedure](release-candidate-qa.md) for exact candidate and signed-tag assets.
