...
- A mixed-protocol model is sent to the wrong Responses, Chat Completions, Anthropic Messages, or Gemini path; the credential is attached under the wrong header convention; or a catalog-only model is presented with a guessed route or favorable data-use policy.
