...
- Foreground compaction activity is ephemeral, server-owned presentation state bound to an exact session and run; subscribers receive current activity after durable replay, and activity never authorizes execution or retry.
...
- `read` returns bounded text or a normalized bounded image. `write` and `edit` apply directly to the requested filesystem target and report only the result Morons can establish. A failed read-only preflight before opening a mutation target is a known tool failure; errors after a possible mutation remain uncertain. Preflight is not confinement, race prevention, rollback, or an invitation to retry uncertain effects.
...
New runs expose six main-agent tools and reject `task` before dispatch. Stored settings cannot authorize delegation. Historical task bindings, child journals, and results remain validated and readable; startup interrupts unfinished children before their outer tools and runs, without replay. The following constraints describe retained historical evidence, not available execution capabilities.

- Child requests disclose remaining budgets and reserve the last provider response for a tools-disabled partial report; forced budget reports retain resource-limit status. No hard limit is extended.
- A child may correct one allowlisted normalization rejection before execution, using only trusted diagnostic feedback and previously accepted history. Rejected calls and their opaque continuation are omitted, never executed or replayed; provider failures, uncertain effects, integrity failures, and resource validation failures remain terminal. Earlier completed effects remain in place.
- Historical `task` calls accepted one explicit bounded shared context and one to three bounded self-contained assignments. No current subagent admission surface exists.
...
- Each child has a stable opaque provider-scoped conversation identifier distinct from its parent and siblings. It is derived from parent session and canonical task-call identity, never exposed, and follows all existing inference-header redaction rules.
...
- A small coding core encourages the selected main model to inspect, plan, implement, and verify directly; no task delegation tool is available.
...
- Each new tool-enabled run commits its instruction snapshot and run-bound digest atomically with input acceptance. Exact retries, later turns and restart recovery never rediscover that run's guidance; historical child snapshots remain pinned. Compaction does not summarize it. Independent context guards charge rendered guidance bytes, and usage observations require equal instruction snapshots.
...
- Every root provider dispatch binds an exact canonical source-entry high water, selected model, context-policy version, limits, active skill set, and pinned project context. Each historical task-child dispatch instead bound the committed parent run selection, canonical outer task-call identity, child index, explicit shared context, assignment, and child limits.
...
- The policy reserves capacity for output, tool contracts, developer context, skills, and bounded tool-loop growth before dispatch. Entry and image count/byte pressure trigger compaction independently of the soft token threshold.
- A full-history admission may saturate its estimated-input counter at the model limit solely to permit pre-dispatch compaction. It does not authorize oversized inference; actual dispatch budgets are revalidated. Manual compaction remains available when an old prefix fills ordinary context.
- If an old completed prefix no longer fits, Morons compacts it before a later dispatch. ADR0052 execution policy1 additionally permits bounded completed-batch cuts within a new native root run, with exact current user intent retained and charged, latest batch retained, four-attempt cap, advancing source and no uncertain replay. Legacy runs keep whole-current-run protection. If mandatory retained information cannot fit, fail durably without hidden truncation. Unexpected persistence failures request server shutdown; they do not publish fabricated terminal outcomes.
- Checkpoint source hashing streams bounded canonical pages. Startup and external SQLite changes invalidate/recheck the worker's integrity proof; new checkpoints require a fresh full-prefix digest binding. Normal context loads materialize only the active suffix.
- Oversized compaction prefixes use bounded, explicitly marked excerpts with complete-prefix source digests. Excerpt loss never changes canonical history, authorizes tools, or includes context-excluded command text. See [ADR 0022](../adr/0022-release-context-and-provider-hardening.md).
- Background maintenance has a separate bounded job/transition ledger, not a fabricated user input or top-level run. Readiness alone never installs a checkpoint; installation requires an atomic first-provider-request boundary check of the receiving run, expected parent, full source, exact profile and every conservative tail guard. Interrupted preparation is cancelled and interrupted dispatch is uncertain, never replayed; valid stale readiness is discarded and corrupt evidence fails closed. Background scheduling is enabled by default with cost disclosure and server-startup owner opt-out (`MORONS_BACKGROUND_COMPACTION=0`); empty/invalid overrides disable it. It can incur extra cost even when its result is discarded. One bounded supervisor owns the request through cancellation/shutdown. Preparation and dispatch recheck idle scope, exact credential/model/profile binding and shared attempt limits; no input is fabricated or queued behind a provider request. See [ADR 0026](../adr/0026-background-compaction-maintenance.md).
- Automatic compaction cannot repeat an already attempted source cut through another maintenance job or foreground run. Foreground compaction may proceed after a maintenance job failed or was cancelled before dispatch; the durable job state must prove no dispatch occurred. Background scheduling still counts these jobs as attempts to prevent idle retry loops. Explicit owner `/compact` input is required to repeat any other attempted cut; it does not authorize an automatic retry after another uncertain dispatch.
...
- Legacy provider usage remains advisory under ADR0023. ADR0052 execution policy1 permits a positive, compatible immediately preceding committed same-run native text request to inform admission; source bytes, reserved entries/items, images, schemas/nodes, continuation and encoding remain independently bounded. Missing/uncertain/wrong-scope evidence never creates credit. The durable accounting epoch and accepted profile select policy independently of source digest4 and returned aliases. Context status labels provenance and source bounds without image I/O or preparation; it is not a billing or attachment-integrity attestation.
...
- ADR0053 removes cumulative parent/child turn, tool, mutation, Task and result-byte quotas and whole-run/batch deadlines. Runs can consume unbounded aggregate time, disk and paid usage until stopped; context, individual operation timeouts/payloads, concurrency, storage, checked accounting and cancellation/draining remain enforced. No uncertain effect is retried.
- No hidden memory crosses sessions. Embeddings, vector storage, and automatic project memory are outside the MVP. Historical `task` children received only explicit ephemeral parent-scoped handoffs and returned only bounded canonical reports.
...
- Model inference transports (main agents and shared compaction requests) may retry only confirmed connection-establishment failures before HTTP request transmission, at most three times after the initial attempt with cancellable 250/500/1000ms backoff inside one total deadline. The exact request and credential binding stay fixed; HTTP errors, ambiguous transport failures, timeouts, streams, tools, web search, OAuth, and restart recovery are never replayed by this policy.
...
- Authenticated local IPC remains the only application transport. ADR 0019 permits only an ephemeral, bounded loopback OAuth callback, not a general HTTP application transport; only authenticated owner requests may start the supervised OAuth core; tools cannot.

## Provider credentials and model egress

- ADR 0019 admits a native ChatGPT OAuth core at fixed reviewed authorization/token routes with S256 PKCE, independent random state, exact loopback callback validation, bounded non-retried exchanges and redacted zeroizing owned buffers. ADR0032 bounds/discards authorization-response extensions per RFC6749, rejects every duplicate decoded name, and pins any supplied issuer exactly; metadata never changes routing, scope grants, credentials, model admission or policy. Browser rejection reasons are closed fixed labels, never reflected callback targets/field names/values. ADR0033 discards bounded token-response extensions without routing/policy authority and accepts Bearer case-insensitively, retaining strict duplicates, required fields, scope, expiry, account and refresh checks. Only closed token-validation reasons may cross the initiating authenticated login connection in IPC43, never provider values or parser errors. It imports no other-app credentials. Provider-specific custody and guarded refresh back a supervised, connection-owned login dialog. ADR 0030 connects native inference through explicit provider-local root, task and compaction identity, with current data-use admission; browser/subscription interoperability still needs deliberate owner qualification.
- [ADR 0028](../adr/0028-native-codex-responses-contract.md) reviews a separate native Codex adapter before application admission. It accepts only its fixed HTTPS route and reviewed full-Responses manifest (GPT-5.5 plus the five exact additions in [ADR 0035](../adr/0035-reviewed-native-model-expansion.md)), exact provider/turn/credential binding and receipt-bound reasoning. Unverifiable account data use fails either restriction. Output limits are local acceptance limits, not backend generation or spending caps; uncertain/dropped dispatch poisons its turn without replay. Durable current-policy/root/child/maintenance admission remains required before enabling models.
- OAuth URLs are ephemeral browser-interaction data, not ordinary logs/status/Morons history. [ADR 0031](../adr/0031-login-browser-and-clipboard-interaction.md) permits automatic default-browser handoff only for the authenticated dialog's fixed validated URL, plus explicit full-link clipboard copy and clickable controls. OS launcher arguments/browser history/clipboard managers may retain navigation URLs; codes, tokens and verifiers are never supplied. Helper ownership is bounded and scoped to the dialog, with no shell source, generic hyperlinks, clipboard reads, automatic copy, OAuth retry or erasure guarantee. Codes and tokens never cross application IPC. Token claims from the fixed TLS exchange supply bounded account routing/expiry only; no cryptographic JWT, entitlement, training or retention assurance is inferred. Dedicated OpenAI state and reused provider-scoped mutation audit precede application admission. Refresh marks its exact source revision dispatched durably before transmission; uncertain/abandoned refresh requires reauthentication without replay, while status queries never interrupt live refresh ownership. No new API-key/token import surface is admitted.

- Only trusted server provider code reads Morons-managed provider credentials or attaches provider authorization headers.
...
- Manual API-key input crosses IPC only after mutual authentication, is collected without echo, uses redacted secret-bearing types, and is never automatically retried after an unknown mutation outcome. Native OAuth accepts authenticated owner initiation, not pasted codes or tokens; callback/exchange secrets stay server-side.
- One owned login connection receives an ephemeral bounded URL and terminal result, never session events. Cancel/disconnect/shutdown drains the supervised attempt; cancellation during installation may report a completed save, not rollback. Lost storage acknowledgement is explicitly uncertain and requests fail-closed shutdown. Login never selects a model.
- Framing JSON failures discard parser details before reaching diagnostics; untrusted field names or values must not leak through error display, debug or source chains.
...
- Production provider and web-search requests use fixed reviewed HTTPS origins and paths, disabled redirects, normal certificate and hostname verification, and exact authorization-header scoping. OpenCode Responses and Chat Completions use sensitive bearer authorization; reviewed Zen and Go Anthropic Messages routes use a sensitive `x-api-key` plus a fixed Anthropic protocol version and receive no bearer header; reviewed Zen Gemini routes use a sensitive `x-goog-api-key` and receive neither other credential header.
- Every OpenCode Zen and Go inference request carries one stable `x-opencode-session` value derived from a Morons-owned conversation identity. A root value remains stable across runs, foreground compaction, and tool turns in its durable session. Background compaction uses a separate job conversation identity and never shares a live root/child continuation. Each historical task child received a distinct value stable across that child's turns and derived from the parent session, canonical task-call identity, and child index. Values differ across unrelated conversations, are not raw local locators, and are never attached to public catalog requests or emitted in logs, errors, persistence, or protocol responses.
- Independent default-off Block training use / Require zero data retention settings are durable owner mutations. Current server policy gates model selection, input and every root/compaction dispatch admission; historical child admissions retain their policy bindings; unknown policy is not favorable. A change cannot recall already admitted effects. Background preparation pins the policy sequence and stale work cannot dispatch/install. See [ADR 0029](../adr/0029-provider-admission-and-data-use-policy.md); [ADR 0030](../adr/0030-native-provider-and-task-bindings.md) supplies the native provider/task identity binding and preserves that admission boundary.
- Remote catalogs may narrow but never enlarge the reviewed model, protocol, capability, limit, route, or data-use manifest. A model identifier appearing remotely is not enough to select Responses, Chat Completions, Anthropic Messages, Gemini, an image capability, or a favorable training/retention label.
...
- Provider requests, responses, streams, errors, usage, identifiers, tool arguments, attachments, and output are bounded, strictly decoded, and sanitized at application boundaries. Protocol-specific request conversion and stream state machines normalize only reviewed Responses, Chat Completions, Anthropic Messages, or Gemini shapes into provider-neutral outcomes.
- Native reasoning provenance retains only fingerprints from the validated current request and its successful response; older reasoning still in active context remains receipt-bound to the same turn, while omitted reasoning is forgotten.
- ADR0037 permits only an omitted response Content-Type on HTTP200 at the fixed native SSE route. Explicit conflicting/empty/duplicate types still reject. Absence selects the same strict bounded decoder, never JSON/HTML fallback, sniffing or a retry; it cannot bypass complete output, model/sequence/usage/receipt checks. OpenCode/OAuth media-type requirements and all limits remain unchanged.
- ADR0038 lets only native Responses collect complete output_item.done values per HTTP response, bounded by128 indices/1MiB source bytes/100,000 source nodes. A valid terminal identity/model/status/usage and complete contiguous items are required; empty terminal output cannot complete from deltas/added items alone. Both present output sources must normalize identically under the old validators. Conflicts, gaps, later deltas for closed items and incomplete streams fail without tool execution, persistence, cross-response reuse or retry. OpenCode keeps its old constructor and acceptance.
- ADR0046 separates consulted-source metadata from returned citations: at most128descriptors per response, still within the old aggregate source/node/URL bounds; at most10returned citations,8queries per search action and8hosted actions remain unchanged. New catalog13-only stages distinguish these count guards without raw values; older catalog12 SearchAction diagnostics stay ambiguous. Source metadata never becomes a guessed citation, navigation authority or spending limit.
- ADR0045 permits only closed stage/category enums in a catalog12-or-later reviewed bound root-web or outer-task Uncertain result. They describe where execution/validation stopped, not remote eligibility or a proven ultimate cause. Root/task uncertainty, no next parent request, bounded sibling drain and no replay remain unchanged; storage failure still dominates. No raw strings or new IPC channel are added, and historical generic Uncertain results are not rewritten or diagnosed retroactively.
- Provider-output rejection diagnostics use only fixed server-owned category labels and bounded enum/boolean classifications. They never log raw bodies, text, arguments, paths, provider identifiers, credentials or opaque continuation data. Diagnostics do not relax decoding or authorize a retry.
- Startup timing uses the same explicit `--debug` sink: fixed stage enums, begin/end flags, success booleans and elapsed microseconds only; no paths, SQL, errors or credentials. Timings do not skip validation/recovery, change deadlines or authorize retries.
- ADR0049 admits explicit foreground `morons-server --debug` only: a bounded nonblocking producer/optional writer emits closed prefixed JSON metadata outside the normal UI, IPC, transcript, model context and SQLite. Default startup creates no writer or log file. At most512 records of1024bytes pass through a32-record queue; stderr failure, backpressure or shutdown may lose records and never changes inference/retry semantics. Guard shutdown waits at most100ms; a blocked writer ends with the process. Local session/task/child/attempt locators are not provider identities or authorization. No raw payload/header/text/path/field fingerprint/credential/reasoning/environment enters a record, and a missing receipt never means zero service usage.
- ADR0050 adds opt-in-only closed runtime resource/normalization categories and returned child-tool metadata, correlated by local run or session/task/index locators and independent attempt IDs. No global last-error state, provider identities, raw tool data, historical diagnostic replay, canonical-validation logging, retry or relaxed guard is introduced. Root terminal diagnostics follow the existing durable failure commit; optional Bash exit/signal are metadata, not command-success or rollback evidence.
- ADR0036 adds one optional code-owned native guard stage per ephemeral turn. After a root protocol-failure transaction commits, IPC44 may send a cursor-free diagnostic scoped by typed local session/run locators; the terminal may display that local run locator, never a provider identifier. It is not assistant output, durable history, a diagnostic query endpoint or replayable memory. Delivery catches up terminal evidence first, uses a bounded64-element broadcast, disconnects lagging consumers, and the client rejects foreign/stale/duplicate diagnostic events.
- ADR0048 permits one reviewed native response-name equivalence: a request/binding for exact `gpt-daybreak-blue-latest` may receive exact `gpt-5.6-sol` in lifecycle/completed model fields. The request, safeguards/approval identity, profile, credential scope and canonical attribution remain Daybreak. This static directional pair cannot be enlarged by remote catalogs, case/prefix/date matching or fallback; every other native model and non-native Responses decoder retains exact equality. Response names never choose routing, tools, limits, billing identity or later requests. Historical rejections remain historical and no raw response value is newly exposed.
...
- Web search uses only the fixed reviewed OpenAI-hosted route and the server's ChatGPT credential broker. SQLite32 binds root web tools (and historical task batches) to the separate search model/contract, native generation, policy chronology and exact source before outer dispatch; children cannot borrow an OpenCode generation or adopt a later login. Only the bounded query and fixed instructions enter search requests, not parent transcripts/files/skills. A model can still put previously read information in a query; this is not a secrecy boundary. Current policy admission follows credential preparation and is serialized with policy mutations. No Brave executor/key lookup/fallback, browser cookies, automatic source navigation or retry is available. Completed citations/actions/token receipts stay separate from coding usage; interrupted tasks may not deliver receipts, which does not prove zero use. Root/child search uncertainty stops the owning operation and recovery never replays it. Historical Brave result bytes remain readable without new identity claims. See [ADR0043](../adr/0043-openai-only-web-execution.md).
...
- ADR0051 B admits the reviewed whole-text/part representation into the viewer: shared sanitization, source-byte-bounded windows, bounded visible-part rendering and adjacent-cursor continuation replace destructive display prefixes. Live pressure freezes a contiguous window until gap-free refresh; preview overflow freezes a contiguous ephemeral prefix without cancelling generation. Oversized graphemes use explicit Unicode escapes, not unbounded terminal cells. Server acceptance, canonical entries, context policy, IPC and credential boundaries are unchanged; delivered DTO text is not a new full-result retrieval API. See [the display bounds review](transcript-display-bounds.md).
