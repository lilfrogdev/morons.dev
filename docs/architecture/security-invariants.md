...
- Production provider and web-search requests use fixed reviewed HTTPS origins and paths, disabled redirects, normal certificate and hostname verification, and exact authorization-header scoping. OpenCode Responses and Chat Completions use sensitive bearer authorization; the reviewed Go Anthropic Messages route uses a sensitive `x-api-key` header plus a fixed Anthropic protocol version and receives no bearer header.
...
- Remote catalogs may narrow but never enlarge the reviewed model, protocol, capability, limit, route, or data-use manifest. A model identifier appearing remotely is not enough to select Responses, Chat Completions, Anthropic Messages, an image capability, or a favorable training/retention label.
