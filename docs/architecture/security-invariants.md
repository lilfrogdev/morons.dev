...
- If an old completed prefix no longer fits, Morons compacts it before a later dispatch. If the current run alone cannot fit, the run fails durably rather than silently dropping current information or abandoning an active run. Unexpected persistence failures request server shutdown; they do not publish fabricated terminal outcomes.
...
- Reported provider usage can refine advisory compaction scheduling only when its committed request matches the active model, protocol, tool policy, skill snapshot and checkpoint lineage. It never relaxes independent conservative dispatch bounds. Context status observes metadata without reading image bytes or preparing compaction; status is not an attachment-integrity attestation. See [ADR 0023](../adr/0023-context-observation-and-request-preparation.md).
