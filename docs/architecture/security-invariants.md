...
- Model inference transports (main agents, subagents, and shared compaction requests) may retry only confirmed connection-establishment failures before HTTP request transmission, at most three times after the initial attempt with cancellable 250/500/1000ms backoff inside one total deadline. The exact request and credential binding stay fixed; HTTP errors, ambiguous transport failures, timeouts, streams, tools, web search, OAuth, and restart recovery are never replayed by this policy.
