...
- Automatic compaction cannot repeat an already attempted source cut through another maintenance job or foreground run. Explicit owner `/compact` input is required to repeat that cut; it does not authorize an automatic retry after another uncertain dispatch.
