...
- Provider-output rejection diagnostics use only fixed server-owned category labels and bounded enum/boolean classifications. They never log raw bodies, text, arguments, paths, identifiers, credentials or opaque continuation data. Diagnostics do not relax decoding or authorize a retry.
