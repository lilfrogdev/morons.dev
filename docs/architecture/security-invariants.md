...
- Windows native FFI for AppContainer, ACL, process attributes, and Job Objects must exist only in the target-specific internal `morons-windows-native` adapter; unsafe code remains denied elsewhere and the adapter exposes only the closed helper-owned launch operation.
- A Windows sandbox process must be created suspended with only dedicated handles, assigned to its configured non-breakaway Job Object before resume, and must never inherit host standard streams or execute before complete Job ownership.
- Operation AppContainer SIDs and ACL grants may reach only operation-private staging; shared images, packaged executables, control roots, and persistent trusted state must never accumulate operation ACL entries.
