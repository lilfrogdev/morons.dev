...
- Python executable and `jupyter_client`/`ipykernel` versions;
- whether a Brave Search key is available.

Verify that the source checkout is clean and that the archive digest matches the candidate manifest. Extract into a new directory and verify every manifest entry, including matching `morons` and `morons-server` executables. On Unix, both executables must have executable mode. Do not add the extracted directory to `PATH` for this test.
...
Set `MORONS_PYTHON` to a reviewed Python executable containing `jupyter_client` and `ipykernel`. Preserve the ordinary `PATH` and development environment; do not use `env -i`. If testing successful web search, set `BRAVE_SEARCH_API_KEY` only in the environment inherited by the companion and do not print it.

Launch the extracted `morons` from `QA_REPO` in a Herdr pane with `HOME=QA_HOME` and `MORONS_PYTHON` set on that process. Herdr should only send keys/text and read the visible pane. Long pasted prompts should use bracketed paste. Use `;`, not `&&`, when the pane shell is Nu.
...
| RUN-08 | IPython evaluates a value, preserves a variable across cells/runs, starts in `QA_REPO`, and renders a traceback without ANSI fragments. |
| RUN-09 | With a Python lacking Jupyter packages, IPython fails with guidance naming `jupyter_client`, `ipykernel`, and `MORONS_PYTHON`. |
