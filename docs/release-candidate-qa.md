...
For the update check, use a separately extracted, previously reviewed archive for the same native target. Launch the prior package with the disposable `QA_HOME` and `QA_REPO`, acknowledge onboarding, create a named baseline session and durable transcript entry, then stop its companion with its **old matching client**. Launch the new package from its own complete extraction directory with the same disposable home and repository. Current source uses **IPC45/SQLite33**; old IPC44 clients cannot control it. A schema32 update creates a database-only schema32 backup and an accounting epoch; old accepted native runs remain legacy and new eligible runs use policy1. The backup excludes credentials, IPC keys, attachment bytes and selected project files. Never mix executables, run old/new companions concurrently, or downgrade migrated state. Record both source commits and archive hashes; use [the current upgrade guidance](testing.md#current-state-and-upgrades).
...
| RUN-26 | Representative implementation requests use the selected main model to inspect, plan, implement, and verify directly. Include a follow-up approving a proposal, a correction to an active task's requirements, and an investigation request: expect concrete work or an evidenced blocker, not agreement alone. Include discussion-only questions and unapproved destructive/external actions as controls: do not expect automatic execution. Record actual model behavior separately from prompt-composition tests; no prompt guarantees compliance. |
...
| RUN-28 | Follow the [explicit-debug procedure](testing.md#opt-in-provider-debugging): default silence, startup-only `morons --debug`, private bounded daily files, authenticated `--debug-status` without starting a server, refusal to change an existing server's mode, and clean stop. No ordinary TUI panel is added. Diagnostic loss is unknown, not zero provider usage. Any live model scenario is separately authorized. |
